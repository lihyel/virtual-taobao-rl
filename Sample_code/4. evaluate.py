...
def load_mdp_p():
    ...
    mdp_p.load_model(
        actor_path="./models/ddpg_actor_virtualTB_MDP(G)_xavier",
        critic_path="./models/ddpg_critic_virtualTB_MDP(G)_xavier",
    )
    return mdp_p
...
def load_mlp():
    ...
    mlp.load_model(
        actor_path="./models/ddpg_actor_virtualTB_MLP_xavier",
        critic_path="./models/ddpg_critic_virtualTB_MLP_xavier",
    )
    return mlp
...
def infer_mdp_g(model_mdp_g, kmeans, state):
    # state = kmeans.cluster_centers_[kmeans.predict([state])[0]]
    # state = torch.Tensor([state]).to(device)
