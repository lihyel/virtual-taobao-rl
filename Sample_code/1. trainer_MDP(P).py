...
def main():
    ...
    # pd.DataFrame({"rewards": rewards}).to_csv("./output/mdp(p)_train_reward.csv")
    # pd.DataFrame({"val_rewards": val_rewards}).to_csv("./output/mdp(p)_val_rewards.csv")
    # pd.DataFrame({"val_ctr": val_ctr}).to_csv("./output/mdp(p)_val_ctr.csv")

    np.save("./output/mdp(p)_value_loss_uniform", np.array(value_losses))
    np.save("./output/mdp(p)_policy_loss_uniform", np.array(policy_losses))
    env.close()
    agent.save_model("virtualTB", "MDP(P)_uniform")
