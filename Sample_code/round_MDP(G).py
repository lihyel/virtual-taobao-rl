...
def main(args):
    ...
    if not os.path.exists(f"./models/kmeans-{n_clusters}.pkl"):
        ...
        with open(f"./models/kmeans-{n_clusters}.pkl", "wb") as f:
    ...
    columns = list(range(88)) + list(range(88 + 2 * (10 - num), 88 + 10 * 2)) + [88 + 10 * 2]


    for i_episode in range(1000):
        state = env.reset()[columns]
    ...
    # pd.DataFrame({"rewards": rewards}).to_csv(f"./output/mdp(g)-回合数{_round}-train-reward.csv")
    # pd.DataFrame({"val_rewards": val_rewards}).to_csv(f"./output/mdp(g)-回合数{_round}-val-rewards.csv")
    # pd.DataFrame({"val_ctr": val_ctr}).to_csv(f"./output/mdp(g)-回合数{_round}-val-ctr.csv")

    # np.save(f"./output/mdp(g)-回合数{_round}-value-loss", np.array(value_losses))
    # np.save(f"./output/mdp(g)-回合数{_round}-policy-loss", np.array(policy_losses))
    env.close()
    agent.save_model("virtualTB", f"MDP(G)-回合数{_round}")
