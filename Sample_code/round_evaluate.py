...
def load_mlp(_round):
    ...
    mlp.load_model(
        actor_path=f"./models/ddpg_actor_virtualTB_MLP-round{_round}",
        critic_path=f"./models/ddpg_critic_virtualTB_MLP-round{_round}",
    )
    return mlp
...
def eval_mdp_p(epoch, _round):
    model_mdp_p = load_mdp_p(_round)
...
def eval_mdp_g(epoch, _round):
    model_mdp_g = load_mdp_g(_round)
...
def eval_mlp(epoch, _round):
    model_mdp_p = load_mdp_p(_round)
    model_mdp_g = load_mdp_g(_round)
    model_mlp = load_mlp(_round)
    with open(f"./models/Kmeans-100.pkl", "rb") as f:
        kmeans = pickle.load(f)
...
def main(argv):
    _round = argv[0]
    eval_mdp_p(5000, _round)
    eval_mdp_g(5000, _round)
    eval_mlp(5000, _round)
