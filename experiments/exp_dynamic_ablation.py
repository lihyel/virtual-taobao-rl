"""
=======================================================================
experiments/exp_dynamic_ablation.py  —  동적 변수 유효성 검증 (Ablation)
=======================================================================
...
[출력 파일]
  ...
  │   ├── seed_{N}/eval_result.csv
  │   └── summary.csv
  ├── exp_dynamic_ablation.log
  ├── ablation_comparison.csv   ← A vs B 비교표
  └── ablation_comparison.png   ← 시각화

[실행]
  python experiments/exp_dynamic_ablation.py
  python experiments/exp_dynamic_ablation.py --seeds 0 1 2 3 4 5
  python experiments/exp_dynamic_ablation.py --eval_only
=======================================================================
"""
...
def train_one_seed(seed, n_episodes, device, group_extractor,
    ...
    encoder = DistilBERTEncoder(
        trainable=not is_mac, device=device, max_length=128)
...
def main():
    ...
    parser.add_argument("--output_dir", default="output/exp_dynamic_ablation")
    ...
    logging.basicConfig(
        ...
        handlers=[
            ...
            logging.FileHandler(
                f"{args.output_dir}/exp_dynamic_ablation.log"),
        ],
    )

    device = get_device()
    logging.info("="*55)
    logging.info("동적변수 유효성 검증 실험 (Ablation Study) 시작")
