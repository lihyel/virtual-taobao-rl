...
class LMDDPG:
    def __init__(self, gamma, tau, hidden_size, action_space, encoder, device):
        ...
        # ★ [수정 3] Critic LR 하향 조정: 1e-3 → 1e-4
        # FT에서도 Q값 오버슈팅 방지 필요 (Frozen과 동일하게 적용)
        # Actor FC LR(1e-4)과 OUNoise sigma(0.2)는 FT 특성상 그대로 유지
...
def train_one_seed(seed, n_episodes, device, group_extractor,
    ...
    # 빈 버퍼 상태에서 랜덤 경험이 가중치를 오염시키는 것을 방지.
    # batch_size(32) × 4 = 128 step 분량 쌓인 후 업데이트 시작
    WARMUP_STEPS = 32 * 4  # 128 step

    rewards, val_rewards, val_ctr = [], [], []
    total_steps = 0  # warmup 카운터
    ...
    for i_ep in range(n_episodes):
        ...
        while True:
            ...
            # clamp 후에도 numpy 변환 시 부동소수점 오차로 경계를 벗어날 수 있으므로
            ...
            state_91d   = next_91d
            ep_reward  += reward
            total_steps += 1  # warmup 카운터 증가

            # ★ [수정 2] warmup 완료 후에만 업데이트
...
def main():
    ...
    parser.add_argument("--eval_only", action="store_true",
                        help="학습 건너뛰고 평가만")
    parser.add_argument("--output_dir", default="output/T2_FT",
                        help="결과 저장 루트 디렉토리 (Frozen 결과와 분리)")
    args = parser.parse_args()

    os.makedirs(args.output_dir, exist_ok=True)

    # ★ [수정 6] CUDA_VISIBLE_DEVICES는 torch 사용 전(파일 상단)에 설정해야 하므로
    # 여기서는 torch.cuda.set_device()로 GPU 지정 (이미 import된 torch에 적용)
    ...
    logging.basicConfig(
        ...
        handlers=[
            ...
            logging.FileHandler(f"{args.output_dir}/t2_ft_multiseed.log"),
        ],
    )
